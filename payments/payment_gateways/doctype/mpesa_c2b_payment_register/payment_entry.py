...
#Code reducing functions
#Input validation
# def validate_input(data):
#     required_fields = ["customer", "company", "currency", "pos_profile_name", "pos_opening_shift_name"]
#     for field in required_fields:
#         if not data.get(field):
#             frappe.throw(_(f"{field.replace('_', ' ').title()} is required"))

# #Process Mpesa Payments
# def process_mpesa_payments(data, errors):
#     new_payments_entry = []
#     if data.get("pos_profile").get("posa_allow_mpesa_reconcile_payments") and data.get("total_selected_mpesa_payments") > 0:
#         for mpesa_payment in data.get("selected_mpesa_payments"):
#             try:
#                 new_mpesa_payment = submit_mpesa_payment(mpesa_payment.get("name"), data.get("customer"))
#                 new_payments_entry.append(new_mpesa_payment)
#             except Exception as e:
#                 errors.append(e)
#     return new_payments_entry

# #process ew payments
# def create_new_payments(data, company, customer, currency, today, pos_opening_shift_name, errors):
#     new_payments_entry = []
#     if data.get("pos_profile").get("posa_allow_make_new_payments") and data.get("total_payment_methods") > 0:
...
# def reconcile_payments_with_invoices(data, all_payments_entry):
#     if data.get("total_selected_invoices") > 0:
#         if data.get("pos_profile").get("posa_allow_reconcile_payments") and data.get("total_selected_payments") > 0:
#             for selected_payment in data.get("selected_payments"):
#                 all_payments_entry.append(selected_payment)

#         # Sort payments and invoices
#         all_payments_entry = sorted(all_payments_entry, key=lambda k: getdate(str(k.get("posting_date"))), reverse=True)
#         all_invoices_list = sorted(data.get("selected_invoices"), key=lambda k: getdate(k.get("posting_date")), reverse=True)

#         # Allocate and reconcile entries
#         reconcile_doc = allocate_and_reconcile_entries(data.get("customer"), data.get("company"), all_invoices_list, all_payments_entry)
#         return reconcile_doc
#     return None

# #display results
# def display_results(new_payments_entry, all_payments_entry, data, errors):
#     msg = ""
#     if new_payments_entry:
#         msg += "<h4>New Payments</h4>"
#         msg += create_table_html(new_payments_entry, "name", "unallocated_amount")

#     if all_payments_entry and data.get("selected_invoices"):
#         msg += "<h4>Reconciled Payments</h4>"
#         msg += create_table_html(all_payments_entry, "name", "unallocated_amount")

#     if data.get("selected_invoices") and data.get("total_selected_invoices") > 0:
#         msg += "<h4>Reconciled Invoices</h4>"
#         msg += create_table_html(data.get("selected_invoices"), "name", "outstanding_amount")

#     if errors:
#         msg += "<h4>Errors</h4>"
#         msg += create_table_html(errors, "Error")

#     if msg:
#         frappe.msgprint(msg)


# def create_table_html(data_list, *fields):
...
#         table_html += "<tr>"
#         for field in fields:
#             table_html += f"<td>{item.get(field)}</td>"
#         table_html += "</tr>"
#     table_html += "</tbody></table>"
#     return table_html

# #allocate and reconcile entries
# def allocate_and_reconcile_entries(customer, company, invoices_list, payments_list):
#     reconcile_doc = frappe.new_doc("Payment Reconciliation")
#     reconcile_doc.party_type = "Customer"
#     reconcile_doc.party = customer
#     reconcile_doc.company = company
...
#     errors = []
#     new_payments_entry = process_mpesa_payments(data, errors)
#     new_payments_entry += create_new_payments(data, company, customer, currency, today, pos_opening_shift_name, errors)

#     all_payments_entry = []
#     reconcile_doc = reconcile_payments_with_invoices(data, all_payments_entry)

#     display_results(new_payments_entry, all_payments_entry, data, errors)
